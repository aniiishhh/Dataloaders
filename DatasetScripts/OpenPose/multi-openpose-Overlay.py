...
Execute using:
    python multi-openpose-Overlay.py --i Input 
"""
...
parser = argparse.ArgumentParser(description="Openpose with overlay")
parser.add_argument("--i", help="Path to the directory containing input images.")
parser.add_argument(
    "--o",
    default="./Output_Overlay",
...
for image_file in image_files:
    ...
    for i in range(nPoints):
        for j in range(len(detected_keypoints[i])):
            cv2.circle(
                frameClone, detected_keypoints[i][j][0:2], 5, colors[i], -1, cv2.LINE_AA
            )
